...
🛠 **Что ты делаешь:**
1. Сначала создай **ровно %s вопросов** по указанной теме.
2. Каждый вопрос должен быть:
    - Составлен на **грамотном русском языке**, без грамматических и стилистических ошибок.
    - Основан **исключительно на тексте Библии (канон)**. Не используй апокрифы, предания или народные версии. Например:
    - Не указывай имя женщины с кровотечением (в Библии оно не названо).
    - Не добавляй персонажей или события, которых **нет в тексте Писания**.
    - Сформулирован понятно, строго и точно.
3. После создания всех вопросов выполни **самопроверку на соответствие канону**:
- Подтверди, что каждый вопрос и каждый правильный ответ можно подтвердить цитатой или фактом из Библии.
- Если не уверен — замени такой вопрос на более достоверный.
4. Только после этого — **сформулируй заголовок**, отражающий суть всех вопросов.
...
- Вариант A (✅ / ❌)  
- Вариант B (✅ / ❌)  
- Вариант C (✅ / ❌) *(если нужно)*  
- Вариант D (✅ / ❌) *(если нужно)*
...
Заголовок теста: *(логичный и точный, охватывающий все вопросы)*

---

📌 **Жесткие правила:**
- Не используй никаких данных из предыдущих сообщений или прошлых викторин. Игнорируй весь внешний контекст.
- В каждом вопросе — **только один правильный ответ** (отмечен ✅).
- Каждый ответ — **не длиннее 15 символов**.
- Количество вариантов ответа — от **2 до 4**.
- Не используй символы *звёздочек*, markdown, кавычки и лишние пояснения.
- Никаких приветствий, заключений или комментариев. Только вопросы, ответы и заголовок.
- Отвечай **исключительно на русском языке**.
- Все формулировки должны быть:
    - грамматически правильны,
    - стилистически чисты,
    - строго соответствовать Библии.
...
🎯 Твоя цель — создать краткую, чёткую, **канонически точную** викторину без лишнего текста."""
    ...
    def makeQuizAi(count_questions, theme, prev_questions: list = None):
        ...
        if prev_questions:
            content_sys += "\n\n⚠️ **Не повторяй эти вопросы в тесте:**\n"
    ...
    def parse_quiz(quiz_text: str, theme):
        ...
        for line in lines:
            ...
            # Обработка строки с вопросом: начинается с цифры и точки, например "1. Какой вопрос?"
