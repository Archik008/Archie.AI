...
- Цитируй строго по тексту Библии в переводе НРП. Без искажений, без вставок, без перефразирования.
- Стихи должны быть полными. Нельзя сокращать или изменять текст.

📄 Формат вывода (строго соблюдать):
Стих дня: <полный текст стиха или стихов>  
Ссылка на стих: <название книги, глава и стих(и) — например: Притчи 3:5–6>

📌 Обязательные правила:
- Строго соблюдай формат: две строки, начинающиеся с `Стих дня:` и `Ссылка на стих:`.
- Без пояснений, мыслей, приветствий или комментариев.
- Без дополнительных символов: **звёздочки**, кавычки, markdown и т.д.
- Только русский литературный язык. Ни одного иностранного слова.
- Никаких грамматических ошибок или отступлений от оригинала.
...
Ссылка на стих: Притчи 3:5"""
    ...
    async def getDailyVerse():
        ...
        # Ищем две строго определённые строки:
        verse_match = re.search(r"^Стих дня:\s*(.+)", response_text, re.MULTILINE)
        reference_match = re.search(r"^Ссылка на стих:\s*(.+)", response_text, re.MULTILINE)

        if not verse_match or not reference_match:
...
📜 Источник: только канонический текст НРП.  
🚫 Переводы, апокрифы, пояснения, интерпретации — запрещены.

📌 Каждый вопрос:
- Строго по НРП: событие, действие или высказывание.
- Подтверждён конкретным стихом.
- Без повторов, даже частичных, из списка запрещённых вопросов (если есть).
- Чётко сформулирован как вопрос, на литературном русском.
- Без архаизмов, церковнославянских форм и сокращений.

📌 Каждый ответ:
- Один правильный (✅) — дословная цитата или фрагмент (до 12 слов), без повторов из вопроса.
- Остальные (❌) — правдоподобные, но неверные, не совпадают по смыслу.
- Все ответы — без латиницы, скобок и лишних символов.
- Никаких английских слов, даже букв.
...
🎯 Цель:
Создай точную, краткую и канонически верную викторину. Только по НРП. Без украшений и повторов.
"""
    @staticmethod
